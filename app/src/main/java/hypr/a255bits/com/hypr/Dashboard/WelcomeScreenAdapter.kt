...
class WelcomeScreenAdapter(val generators: MutableList<BuyGenerator>, val context: Context) : RecyclerView.Adapter<WelcomeScreenAdapter.CustomViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CustomViewHolder?, position: Int) {
        ...
        val imageName = generators[position].imageLocation
        val image: BitmapDrawable = ContextCompat.getDrawable(context, context.resources.getIdentifier(imageName, "drawable", context.packageName)) as BitmapDrawable
        holder?.image?.setImageBitmap(image.bitmap)
